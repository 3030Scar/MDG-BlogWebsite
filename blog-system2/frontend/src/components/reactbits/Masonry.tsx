...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const skipFancyAnim = isTouchRef.current;
    ...
    grid.forEach((item, index) => {
      ...
      if (!hasMounted.current) {
        if (skipFancyAnim) {
          // Mobile: simple fade-in, no blur or off-screen animation
          gsap.fromTo(
            selector,
            { opacity: 0, ...animProps },
            { opacity: 1, ...animProps, duration: 0.4, ease: "power2.out", delay: index * 0.02 }
          );
        } else {
